...
$command = '';
$emptyResult = '["TSref",["TSref","stdWrap"],["TypoScript Reference", "stdWrap"],["http://wiki.typo3.org/TSref","http://wiki.typo3.org/TSref/stdWrap"]]';

// Logging
$fp = fopen('fileadmin/suggestion.txt','a');
fwrite($fp, ''.date('Y.m.d. H:i:s').';'.$_GET['command']."\r\n");
fclose($fp);

// Suggestion Help
$help = '["help",["mm | search in mittwald medien",
		"ext | search in typo3.org/extensions",
		"? | this help",
		"wiki | search in wiki.typo3.org",
		"any typoscript Object or function",
		"forge | search in forge.typo3.org",
		"api | search in the api",
...
if (isset($_GET['command']) && strlen($_GET['command']) > 0) {
...
// erste Tests um via suggestion existierende TypoScript Funktionen/Eigenschaften aufgelistet zu bekommen
$stdWrap = array();
$stdWrap[] = array('keyword' => '.cObject','description' => 'cObject','url' => 'http://wiki.typo3.org/TSref/stdWrap#cObject');
$stdWrap[] = array('keyword' => '.case','description' => 'case = lower / upper','url' => 'http://wiki.typo3.org/TSref/stdWrap#case');

$suggestions = array();
// $suggestions[] = array('keyword','path','description','url');
$suggestions[] = array('keyword' => 'TEXT','path' => 'TEXT','description' => 'TEXT Object','url' => 'http://wiki.typo3.org/TSref/TEXT');
$suggestions[] = array('keyword' => 'HTML','path' => 'HTML','description' => 'HTML Object','url' => 'http://wiki.typo3.org/TSref/HTML');
$suggestions[] = array('keyword' => 'HTML.value','path' => 'HTML.value','description' => 'string / stdWrap','url' => 'http://wiki.typo3.org/TSref/HTML');
$suggestions[] = array('keyword' => 'HTML.value.stdWrap','path' => 'HTML.value.stdWrap','description' => 'string / stdWrap','url' => 'http://wiki.typo3.org/TSref/HTML');

foreach ($stdWrap as $k => $v) {
	$suggestions[] = array('keyword' => 'HTML.value.stdWrap'.$v['keyword'],'description' => $v['description'],'url' => $v['url']);
	$suggestions[] = array('keyword' => 'TEXT.stdWrap'.$v['keyword'],'description' => $v['description'],'url' => $v['url']);
}
...
foreach ($suggestions as $k => $v) {
	if (strtolower(substr($v['keyword'],0,strlen($command))) === strtolower($command)) {
		$keyword[] = '"'.utf8_encode($v['keyword']).'"';
		$description[] = '"'.utf8_encode($v['description']).'"';
		$url[] = '"'.utf8_encode($v['url']).'"';
	}
}

// keywords ?
// Wenn ein bestimmtes Keyword eingegeben wurde, dann wird groß/Kleinschreibung ggf. korrigiert
// 
$keywords = array();
include_once('keywords.inc');
foreach ($keywords as $k => $v) {
	if (!is_array($v)) {
		if (strtolower(substr($v,0,strlen($command))) == strtolower($command)) {
			$keyword[] = '"'.utf8_encode($v).'"';
			$description[] = '"'.utf8_encode($v).'"';
			// $url[] = '"'.utf8_encode($v).'"';
		}
	}
}
...
die('["'.utf8_encode($command).'",['.implode(',',$keyword).'],['.implode(',',$description).'],['.implode(',',$url).'] ]');
