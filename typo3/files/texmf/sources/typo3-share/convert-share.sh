...
if [ ! -z "$MISSING_FONTS" ]; then
	echo "No Truetype fonts (*.ttf) found in current dir (`pwd`)." >&2
	echo "Please put following TYPO3 font files into this directory:" >&2
...
if [ $(which ttf2afm | wc -l) -lt 1 ]; then
	echo "ttf2afm is not available." >&2
	exit 1
fi
if [ $(which ttf2tfm | wc -l) -lt 1 ]; then
	echo "ttf2tfm is not available." >&2
	exit 1
fi
if [ $(which vptovf | wc -l) -lt 1 ]; then
	echo "vptovf is not available." >&2
	exit 1
fi
